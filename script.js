	...
	.then((value) => {
		...
		for (contests of value) {
            ...
            <p class="card-text">
              ...
              <p><b>Contest Date</b>: ${contests["start_time"].slice(8, 10)}-${contests["start_time"].slice(
					5,
					7
				)}-${contests["start_time"].slice(0, 4)}</p>
            </p>
		...
		contest_platform.forEach((contest) => {
			let li = document.createElement("li");
			li.setAttribute("class", "autocomplete_list hidden");
			li.innerText = contest;
			autocompleteBox.appendChild(li);
		});

		let autocompleteListItems = Array.from(document.querySelectorAll(".autocomplete_list"));
		...
		searchBox.addEventListener("input", (event) => {
			...
			for (let i = 0; i < existing_cards; i++) {
				try {
					if (
						card_divs[i].childNodes[6].textContent.slice(6).slice(0, input.length).toUpperCase() !== input
					) {
			...
			autocompleteListItems.forEach((listItem) => {
				if (input.length !== 0) {
					if (listItem.textContent.toLocaleUpperCase().includes(input)) {
...
// To prevent refresh on form submit
